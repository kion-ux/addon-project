...
def item(it: spec.Item) -> dict:
    ...
    if it.hidden:
        # 形態表示体が世界に出てしまうと、変身するたびに増えていく
        # （企画書 §14 アイテム増殖を防ぐ構造 / QA-03）。
        # 死亡時のドロップが一番の漏れ口なので、持ったまま死ねるようにする。
        # item_lock は使わない — スクリプトからの取り外しまで塞がれた場合、
        # 「変身が解除できない」という、もっと悪い壊れ方をするため。
        # プレイヤーが自分で外した場合は、スクリプト側の掃除が拾う。
        comps["minecraft:keep_on_death"] = {}
    if it.wearable:
